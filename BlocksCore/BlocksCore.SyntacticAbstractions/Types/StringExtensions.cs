...
namespace BlocksCore.SyntacticAbstractions.Types
{
   /// <summary>
    ...
    public static class StringExtensions
    {
        ...
        {
            if (string.IsNullOrEmpty(subject) || chars == null || chars.Length == 0) return false;

            for (var i = 0; i < subject.Length; i++)
            {
                var current = subject[i];
                if (Array.IndexOf(chars, current) >= 0) return true;
            }
        ...
        public static bool All(this string subject, params char[] chars)
        {
            ...
            {
                var current = subject[i];
                if (Array.IndexOf(chars, current) < 0) return false;
            }
